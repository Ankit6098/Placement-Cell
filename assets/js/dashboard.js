...
const appliedJobsContainer = document.querySelector(
  ".appliedJobs-main-container"
);
...
function renderJobs(jobs) {
  ...
  element.innerHTML = `
        <div className="company-more-info-upper-container" style="display:flex; gap:1rem">
            ...
            <div class="company-more-info-text">
                ...
                <p class="company-more-info-expreince">${jobs.experience}</p>
  ...
  for (let i = 0; i < jobs.applicantList.length; i++) {
    if (jobs.applicantList[i] == studentId) {
      const applyJobsRequest = document.querySelector(
        ".company-more-info-details-apply-btn"
      );
