    ...
    try {
        ...
        users.forEach((element) => {
            records.push({
                ...
                email: element.email,
                phone: element.phone,
                companyName: element.appliedJobs.map((job) => job.companyName).join(", "),
                jobTitle: element.appliedJobs.map((job) => job.jobTitle).join(", "),
                companyLocation: element.appliedJobs.map((job) => job.companyLocation).join(", "),
                jobSalary: element.appliedJobs.map((job) => job.jobSalary).join(", "),
                jobSkills: element.appliedJobs.map((job) => job.jobSkills).join(", "),
                interviewMode: element.appliedJobs.map((job) => job.interviewMode).join(", "),
                jobDescription: element.appliedJobs.map((job) => job.jobDescription).join(", "),
                companyEmail: element.appliedJobs.map((job) => job.companyEmail).join(", "),
                companyWebsite: element.appliedJobs.map((job) => job.companyWebsite).join(", "),
                interviewStatus: element.appliedJobs.map((job) => job.interviewStatus).join(", "),
            });
        });
